...
public class ElasticSearchUtil {
    ...
    public static boolean createIndex(String indexName , XContentBuilder builder ) {
        /* IndexRequest indexRequest = new IndexRequest();indexRequest.source(builder); indexRequest.source().utf8ToString();
        等价于
        Objects.requireNonNull( BytesReference.bytes(builder)).utf8ToString()*/
        try {
            if(checkIndexExist( indexName)){
                throw new Exception("index 存在");
            }
            HttpEntity entity = new NStringEntity(Objects.requireNonNull( BytesReference.bytes(builder)).utf8ToString(), ContentType.APPLICATION_JSON);
            // 使用RestClient进行操作 而非rhlClient
            Response response = restClient.performRequest("put", indexName, Collections.<String, String> emptyMap(),  entity);
            if (response != null) {
                LOGGER.info("创建索引成功");
            } else {
                LOGGER.info("创建索引失败");
    ...
    public static boolean createMapping(String indexName,String mappingName , XContentBuilder builder ) throws Exception {
        /* IndexRequest indexRequest = new IndexRequest();indexRequest.source(builder); indexRequest.source().utf8ToString();
        等价于
        Objects.requireNonNull( BytesReference.bytes(builder)).utf8ToString()*/
       if(!checkIndexExist(indexName)){
        ...
        try {
            HttpEntity entity = new NStringEntity(Objects.requireNonNull( BytesReference.bytes(builder)).utf8ToString(), ContentType.APPLICATION_JSON);
            // 使用RestClient进行操作 而非rhlClient
            Response response = restClient.performRequest("put", indexName+"/"+mappingName+"/_mapping", Collections.<String, String> emptyMap(),  entity);
            if ( response.getStatusLine().getReasonPhrase().equals("OK")) {
                LOGGER.info("创建mapping成功");
            } else {
                LOGGER.info("创建mapping失败");
                return false;
            }
            return true;
        } catch (UnsupportedEncodingException e) {
